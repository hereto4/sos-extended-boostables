...
abstract class SPlanAbs <T extends ROOM_SERVICE_ACCESS_HASER> extends AIPLAN.PLANRES{
	...
	private final Resumer walk = new Resumer("Walk") {
		...
		protected AISubActivation setAction(Humanoid a, AIManager d) {
			...
			if (include)
				s = AI.SUBS().walkTo.serviceInclude(a, d, blue(d).service(), STATS.FOOD().STARVATION.indu().get(a.indu()) > 0 ? Integer.MAX_VALUE : blue(d).service().radius);
			else
				s = AI.SUBS().walkTo.service(a, d, blue(d).service().finder, STATS.FOOD().STARVATION.indu().get(a.indu()) > 0 ? Integer.MAX_VALUE : blue(d).service().radius);
			if (s == null)
				return null;
