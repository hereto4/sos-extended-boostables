...
public class JobBuildStructure {
	...
	private final class Convert extends PlacableMulti {
		...
		public CharSequence isPlacable(int tx, int ty, AREA a, PLACER_TYPE t) {
			...
			if (count) {
				if (te instanceof TBuilding.Wall && ((TBuilding.Wall) te).building().wall == te && wall.res() != null)
					res += wall.resAmount();
		...
		public void place(int tx, int ty, AREA a, PLACER_TYPE t) {
			if (terrain.isser.is(tx, ty))
				return;
			TerrainTile te = SETT.TERRAIN().get(tx, ty);
			if (te instanceof TBuilding.Wall && ((TBuilding.Wall) te).building().wall == te && allocated + wall.resAmount() < SETT.ROOMS().STOCKPILE.tally().amountReservable.get(wall.res())) {
				terrain.wall.placeFixed(tx, ty);
