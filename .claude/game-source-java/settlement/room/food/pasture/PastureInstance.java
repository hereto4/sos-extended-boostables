...
public final class PastureInstance extends RoomInstance implements JOBMANAGER_HASER, ROOM_PRODUCER_INSTANCE {
	...
	PastureInstance(ROOM_PASTURE p, TmpArea area, RoomInit init) {
		...
		for (COORDINATE c : body()) {
			...
			water += SETT.GROUND().MOISTURE_CURRENT.get(c.x(), c.y());
		};
		...
		skillPrev = (float) blueprintI().bonus().get(POP_CL.clP(null, HCLASSES.CITIZEN()));
	...
	public void updateTileDay(int tx, int ty) {
		waterCount++;
		waterN += SETT.GROUND().MOISTURE_CURRENT.get(tx, ty);
		if (waterCount >= area()) {
