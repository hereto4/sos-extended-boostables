...
final class OTile {
	...
	private final RoomBits bState = 		new RoomBits(coo,	new Bits(0b0000_0000_0000_0000_0000_0000_0000_0111)) {
		@Override
		public void set(int tx, int ty, RoomInstance r, int t) {
			if (false) {
				//count trees here and use it in the bluerint production.
			}
...
//				c.debug();
	...
	private final SETT_JOB job = new SETT_JOB() {
		...
		public RESOURCE jobPerform(Humanoid skill, RESOURCE r, int rAm) {
			...
			bReserved.set(ins, 0);
			double s = IndustryUtil.calcConsumptionRate(1, skill, ins.industry(), ins, ins.industry().outs().get(0).resource);
			ins.incSkill(s);
			int am = (int) s;
			if (s-am > RND.rFloat())
				am++;
