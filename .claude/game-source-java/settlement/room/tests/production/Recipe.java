...
public class Recipe {
	...
	Recipe(int index, Industry ins, IndustryResource out){
		this.ins = ins;
		res = out.resource;
		rate = out.rate;
	...
	public double wPerItem(ProductionSpec ibonuses) {
		return (wPerItem*(1+ibonuses.wPerItemUsed()))/ibonuses.bonus(ins);
	}
	...
	public double wTotPerItem(ProductionSpec ibonuses) {
		...
		for (int i = 0; i < inputs.size(); i++) {
			...
			w += (ii.amount*(1+ibonuses.wPerItemUsed()))*ii.producer.wTotPerItem(ibonuses)/(rate*ibonuses.consumptionBonus(ins));
		}
		return w;
	}
