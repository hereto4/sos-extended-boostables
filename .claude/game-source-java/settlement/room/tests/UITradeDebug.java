...
import settlement.room.tests.production.Recipe;
import snake2d.LOG;
...
class UITradeDebug extends GuiSection{
	...
	private final IntImp oldTariff = new IntImp(0, 100);
	...
	private class BonusPlayer implements ProductionSpec {
		...
		private void recount() {
			...
			final double toll = 0.2;
			final double buyT = 0.4;
			final double sellT = 0;
			...
			while(change) {
				...
				for (Industry ins : SETT.ROOMS().industries.all) {
					...
					for (Recipe r : data.get(ins)) {
						
						
						int in = 0;
						for (Input i : r.inputs()) {
							double amount = spec.bonus(ins)*i.amount;
							in += trade.buyPrice(i.res, spec, amount, buyT, toll);
						}
						
						double amount = spec.bonus(ins)*r.rate;
						
						int out = trade.sellPrice(r.res, spec, amount, sellT, toll);
						...
						if (in > 0) {
							...
							//out - credits = in*c
							//(out-credits)/in = c
							double c = (out-credits)/(double)in;
							LOG.ln(ins.blue.key + " " + r.rate + " " + -out + " " + in + " " + c + " " + (c < 1));
							if (c < 1) {
			...
			LOG.ln();
			for (Industry ins : SETT.ROOMS().industries.all) {
				LOG.ln(ins.blue.key + " " + cons[ins.index()]);
		...
		@Override
		public double consumptionBonus(Industry ins) {
			return (cons[ins.index()]*consumptionBonus.getD());
		}
	...
	UITradeDebug(){
		...
		
		{
			...
			i = new GSliderInt(oldTariff, 100, false);
			i.hoverInfoSet("old sell tariff");
			info.addRightC(48, i);
		...
		
		{
			...
			info.addRightC(0, new GButt.ButtPanel("reset") {
				@Override
			...
			info.addRightC(0, new GButt.ButtPanel("research") {
				...
				protected void clickA() {
					...
					for (Industry ins : Industry.all()) {
						...
						LOG.ln(ins.blue + " " + pbonuses.bonus[ins.index()]);
			...
			info.addRightC(0, new GButt.ButtPanel("consumption") {
				...
				protected void clickA() {
					...
					gametheory.set(20);
					flatTariffSell.set(0);
					flatTariffBuy.set(40);
					bonusai.set(100);
					pbonuses.clear();
					consumptionBonus.setD(1.0);
			...
			public RENDEROBJ build(GETTER<Integer> ier) {
				return new GStat() {
					...
					public void update(GText text) {
						...
						GFORMAT.iIncr(text, trade.sellPrice(r.res, aibonuses, tot, flatTariffSell.getD(), toll.getD()));
					}
