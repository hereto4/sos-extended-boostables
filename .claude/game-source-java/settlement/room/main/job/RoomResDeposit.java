...
public abstract class RoomResDeposit implements SETT_JOB {
	...
	protected RoomResDeposit(RoomBlueprintImp blue) {
		this.blue = blue;
		if (false) {
			//sadly, some industries need to fetch more than they work, meaning the fetch work ratio does not work. Solution for this is to make all workstations fetchable. resources must also be able to "borrow" resources once the job completes.
		}
	}

	public RoomResDeposit get(int tx, int ty, RoomInstance i) {
		if (i != null && i.is(tx, ty) && is(tx, ty) && i instanceof ROOM_PRODUCER_INSTANCE) {
			coo.set(tx, ty);
