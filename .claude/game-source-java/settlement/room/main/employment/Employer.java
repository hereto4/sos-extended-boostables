...
//			}
	...
	private class A_Target implements AA{
		...
		public boolean update() {
			...
			if (!sort.hasMore()) {
				...
				for (Bucket e : buckets) {
					...
					if (e.needed > 0) {
						sort.add(e);
						if (log) {
							LOG.ln("adding: " + e.e.blueprint().key + " " + e.needed);
						}
					}
				}
				if (!sort.hasMore())
			...
			if (log) {
				LOG.ln("avaliable: " + a + "/" + needed + " " + d + " " + sort.hasMore());
			}
	...
	private class A_Assign implements AA{
		...
		void allocate(Bucket e, LIST<WGROUP> same, int prio){
			...
			if (log)
				LOG.ln("ALLOCATING: " +  e.e.blueprint().key + " " + e.target + " " + prio + " " + d + " " + same.size());
		...
		void allocatePref(Bucket e, LIST<WGROUP> same){
			...
			double pref = 0;
			for (WGROUP g : same) {
				pref += e.e.neededWorkers(g);
			}
			

			int toAllocate = e.needed-e.tot();
			if (log)
				LOG.ln("PREF ALLOCATING: " +  e.e.blueprint().key + " " + e.needed + " " + e.tot() + " " + toAllocate + " " + same.size() + " " + pref);
			...
			for (WGROUP g : same) {
				...
				int am = (int) (needed*e.e.neededWorkers(g)/pref);
				am = CLAMP.i(am, 0, e.needed-e.tot());
				am = CLAMP.i(am, 0, available.get(g));
				available.add(g, -am);
				e.add(g, am);
				if (log)
					LOG.ln("AP: " + g.race + " " + g.type + " " + am);
			}			
		}
	...
	private class A_Setter implements AA{
		...
		public boolean update() {
			...
			e.e.target.clear();
			if (log) {
				LOG.ln(e.e.blueprint().key + " " + e.needed + " " + e.target + " " + e.tot());
			}
			for (WGROUP g : WGROUP.all()) {
	...
	private static class Workers {
		...
		private final int[] amount = new int[WGROUP.all().size()];
		private int total = 0;
