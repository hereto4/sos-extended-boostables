...
//	}
	...
	public void setPrioOnSkill(WGROUP g) {
		int p = CLAMP.i((int)Math.ceil(1 + RACES.boosts().getNorSkill(g.race, this)*19), 0, 20);
		priorities.set(g, p);
	}
	
	public void setPrioOnFullfillment(WGROUP g) {
		int p = CLAMP.i((int)Math.ceil(1 + g.race.pref().getWork(this)*19), 0, 20);
		priorities.set(g, p);
	}
	...
	public static class GRoupInt implements INT_OE<WGROUP>{
		...
		private final int[] racePrio = new int[WGROUP.all().size()];
	...
	public static class Target {
		...
		private int target;
		private final int[] perGroup = new int[WGROUP.all().size()];
