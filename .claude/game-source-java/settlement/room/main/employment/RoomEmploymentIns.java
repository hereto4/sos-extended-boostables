...
public final class RoomEmploymentIns extends SecretEmployment implements Serializable{
	...
	public final static int FETCH_FREE_SECONDS = (int) ((Room.MAX_DIM + 20)*2/4.5);
	public final static int DIST_FREE_SECONDS = (int) (TIME.workSeconds()*0.1);
	...
	protected void update(boolean active, boolean day, boolean auto, double seconds) {
		...
		if (day) {
			...
			if (proxCount > 0) {
				double p = walkSeconds/(TIME.workSeconds());
				walkSeconds = 0;
				proxCount = 0;
				p = 1-p;
	...
	private int toolI(RoomEquip w, int ii) {
		if (equipData.length != SETT.ROOMS().employment.equip.ALL.size()*3)
			equipData = new int[SETT.ROOMS().employment.equip.ALL.size()*3];
	...
	public void reportWalkSeconds(int seconds) {
		...
		int max = (int) (employed()*TIME.secondsPerDay()*0.25);
	...
	public double proximitySoFar() {
		if (proxCount == 0)
			return lastProximity/100.0;
		return 1.0 - walkSeconds/(TIME.workSeconds());
	}
