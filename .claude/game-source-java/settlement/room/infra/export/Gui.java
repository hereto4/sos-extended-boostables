		...
		
		{
			...
			final UIPickerRes pop = new UIPickerRes(true) {
				...
				protected void hoverResource(RESOURCE res, GBox b) {
					...
					b.textL(Dic.¤¤Buyers);
					b.tab(6);
					b.textL(Dic.¤¤buyPrice);
					b.tab(9);
					b.textL(Dic.¤¤Stored);
					b.NL();
					for (Faction hh : DIP.traders()) {
						FactionNPC h = (FactionNPC) hh;
			...
			r = new GStat() {
				...
				public void hoverInfoGet(GBox b) {
					...
					if (g.get().resource() != null) {
						...
						b.textLL(Dic.¤¤Sold);
						b.tab(6);
						b.add(GFORMAT.i(b.text(), blueprint.tally.promised.get(g.get().resource())));
						b.NL();
						
						b.textLL(Dic.¤¤Exporting);
						b.tab(6);
						b.add(GFORMAT.i(b.text(), blueprint.tally.attempting.get(g.get().resource())));
						b.NL();
			...
			
			{
				...
				p = new GButt.ButtPanel(UI.icons().m.priority) {
					...
					protected void render(SPRITE_RENDERER r, float ds, boolean isActive, boolean isSelected,
						...
						super.render(r, ds, isActive, isSelected, isHovered);
						if (g.get().resource() == null || (blueprint.prioFetchAvailable(g.get().resource()) < 0 || g.get().prio() && g.get().coolFetch > -1)) {
							GCOLOR.UI().SOSO.hovered.bind();
							UI.icons().s.alert.render(r, body.x1()+6, body.y1()+6);
							COLOR.unbind();
						}
					...
					public void hoverInfoGet(GUI_BOX text) {
						...
						}else if (blueprint.prioFetchAvailable(g.get().resource()) < 0) {
