...
final class EmbassyInstance extends RoomInstance implements JOBMANAGER_HASER, ROOM_IDATA_INSTANCE{
	...
	protected void updateAction(double updateInterval, boolean day) {
		...
		blueprintI().consumption().updateRoom(this);
		{
			...
			for (IndustryResource rr : blueprintI().consumption().ins()) {
				double prod = rr.rateSeconds*skill*time;
				if (prod > blueprintI().consumption().stored(rr).get(this))
					prod = blueprintI().consumption().stored(rr).get(this);
				int am = rr.inc(this, prod);
				blueprintI().consumption().stored(rr).inc(this, -am);
