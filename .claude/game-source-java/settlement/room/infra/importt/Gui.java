		...
		if (S.get().developer) {
			...
			r = new GStat() {
				...
				public void update(GText text) {
					...
					RESOURCE r = i.resource();
					if (r != null) {
						GFORMAT.iBig(text, blueprint.tally.reserved.get(r));
					}
				}
			}.hh("t reserved");
			grid.add(r);
			...
			r = new GStat() {
				...
				public void update(GText text) {
					...
					if (r != null) {
						int am = 0; 
						for (RMapInt<RESOURCE> ii : blueprint.tally.iAll) {
							am+= ii.get(r);
						}
						
						GFORMAT.iBig(text, am);
					}
				}
			}.hh("t to deliver");
			grid.add(r);
...
//			grid.add(r);
		...
		final UIPickerRes pop = new UIPickerRes(true) {
			...
			protected void hoverResource(RESOURCE res, GBox b) {
				...
				p = blueprint.tally.warning(res, true);
				if (p != null) {
				...
				for (Region reg : FACTIONS.player().realm().all()) {
					for (RDResource r : RD.OUTPUT().RES) {
						if (r.res == res) {
							double am = r.boost.get(reg) + r.boostYearlyPart.get(reg);
							if (am > 1) {
