...
final class Bed {
	...
	public static boolean made(int tx, int ty) {
		return self.init(tx, ty) && self.wres1.get() > 0 && self.sstate.get() != self.I_UNAVAILABLE;
	}
	...
	private final SETT_JOB job = new SETT_JOB() {
		...
		public RESOURCE jobPerform(Humanoid skill, RESOURCE r, int rAm) {
			...
			if (r == b().indus.get(0).ins().get(0).resource) {
				...
				if (sstate.get() == I_UNAVAILABLE && wres1.get() > 0) {
