...
public abstract class RoomService  {
	...
	public RoomService(RoomBlueprintIns<?> b, RoomInitData data, NEED need) {
	...
	public double totalMultiplier() {
		...
		if (need != null) {
			...
			if (need instanceof NEED_E) {
				return 1.0/(ne*need.rate.get(POP_CL.clP(null, null)));
			}else {
				double tot = 0;
				for (int ni = 0; ni < NEEDS.ALLSIMPLE().size(); ni++) {
					NEED o = NEEDS.ALLSIMPLE().get(ni);
					tot += o.rate.get(POP_CL.clP(null, null));
				}
				if (STATS.SERVICE().needTot(need) == 0)
					ne = usage;
				return 1.0/(ne*TIME.servicePerDay()*0.5*need.rate.get(POP_CL.clP(null, null))/tot);
